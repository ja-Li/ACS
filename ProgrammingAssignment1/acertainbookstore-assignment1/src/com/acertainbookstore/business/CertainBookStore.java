...
public class CertainBookStore implements BookStore, StockManager {
	...
	public synchronized List<Book> getTopRatedBooks(int numBooks) throws BookStoreException {
		if(numBooks < 0){
			throw new BookStoreException("numBooks" + numBooks + BookStoreConstants.NULL_INPUT);
		}
		// Get all books that are editor picks.
		List<BookStoreBook> listRate = bookMap.values().stream()
				.sorted(Comparator.comparing(BookStoreBook::getAverageRating).reversed())
				.collect(Collectors.toList());
		List<Book> booksRank = new ArrayList<>();
		int n = numBooks;
		while(!listRate.isEmpty() && n > 0){
			booksRank.add(listRate.get(numBooks - n));
			n--;
		}
		return booksRank;
	}
...
//		throw new BookStoreException();
		return bookMap.values().stream().filter(BookStoreBook::hadSaleMiss)
	...
	public synchronized void rateBooks(Set<BookRating> bookRating) throws BookStoreException {
		...
		for (BookRating book : bookRating) {
			validateISBNInStock(book.getISBN());
