...
//	private static boolean localTest = true;
	...
	/**
	 * Tests that a book can be rated if ISBN and rating are valid
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	...
	/**
	 * Tests that books with valid ISBNs can be rated.
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	@Test
	public void testRateABook() throws BookStoreException {
		List<StockBook> booksInStorePreTest = storeManager.getBooks();

		// Try to rate a book with valid ISBN.
		Set<BookRating> booksToRate = new HashSet<BookRating>();
		booksToRate.add(new BookRating(TEST_ISBN, 2)); // valid

		// Try to rate the books.
		client.rateBooks(booksToRate);

		Set<Integer> bookInList = new HashSet<>(Collections.singletonList(TEST_ISBN));
		List<StockBook> booksInStorePostTest = storeManager.getBooksByISBN(bookInList);

		// Check pre and post state are same.
		assertEquals(2, booksInStorePostTest.get(0).getTotalRating());
		assertTrue(booksInStorePreTest.containsAll(booksInStorePostTest)
				&& booksInStorePreTest.size() == booksInStorePostTest.size());
	}
	...
	public void testRateInvalidISBN() throws BookStoreException {
		...
		Set<BookRating> booksToRate = new HashSet<BookRating>();
		booksToRate.add(new BookRating(-1, 2)); // invalid
	...
	/**
	 * Tests that books can only be rate if they are in the book store.
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	...
	public void testRateNonExistingISBN() throws BookStoreException {
		...
		// Try to rate a book with isbn not in stock
		HashSet<BookRating> booksToRate = new HashSet<BookRating>();
		booksToRate.add(new BookRating(TEST_ISBN, 3)); // valid
		booksToRate.add(new BookRating(10000, 1)); // invalid
		...
		List<StockBook> booksInStorePostTest = storeManager.getBooks();
		Set<Integer> bookInList = new HashSet<>(Collections.singletonList(TEST_ISBN));
		List<StockBook> books = storeManager.getBooksByISBN(bookInList);

		// Check pre and post state are same.
		assertTrue(booksInStorePreTest.containsAll(booksInStorePostTest)
				&& 0 == books.get(0).getTotalRating());
		assertTrue(booksInStorePreTest.containsAll(booksInStorePostTest)
				&& booksInStorePreTest.size() == booksInStorePostTest.size());
	}

	/**
	 * Tests that you can't rate with wrong rating.
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	@Test
	public void testRateInvalidRatingBooks() throws BookStoreException {
		addBooks(TEST_ISBN-1,1);
		List<StockBook> booksInStorePreTest = storeManager.getBooks();
		// Try to rate with invalid rating
		HashSet<BookRating> booksToRate = new HashSet<BookRating>();
		booksToRate.add(new BookRating(TEST_ISBN, 3)); // valid
		booksToRate.add(new BookRating(TEST_ISBN-1, 10)); // invalid
	...
	/**
	 * Tests that you can getTopRatedBooks with Invalid number.
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	@Test
	public void testGetTopRatedBooksInv() throws BookStoreException {
		List<StockBook> booksInStorePreTest = storeManager.getBooks();
		// Try to rate with invalid rating
		HashSet<BookRating> booksToRate = new HashSet<BookRating>();
		booksToRate.add(new BookRating(TEST_ISBN, 3)); // valid
	...
	/**
	 * Tests that you can getTopRatedBooks with number 0.
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	@Test
	public void testGetTopRatedBooksWith0() throws BookStoreException {
		assertEquals(Collections.emptyList(), client.getTopRatedBooks(0));
	}

	/**
	 * Tests that you can get the right top rated books.
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	...
	public void testGetTopRatedBooks() throws BookStoreException {
		addBooks(TEST_ISBN-1,1);
		HashSet<BookRating> booksToRate =
				new HashSet<BookRating>(
						Arrays.asList(new BookRating(TEST_ISBN, 3),
								new BookRating(TEST_ISBN, 1),
								new BookRating(TEST_ISBN-1, 4),
								new BookRating(TEST_ISBN-1, 5)));
		// Get books in store.
		client.rateBooks(booksToRate);
